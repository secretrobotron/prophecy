...
def demonstrate_ai_provider_basic_usage():
    ...
    try:
        ...
        ai_provider = AIProviderFactory.create_provider(
            "chatgpt", api_key=api_key, model="gpt-3.5-turbo", max_tokens=500, temperature=0.7
        )
        ...
        if api_key != "mock_key_for_demo":
            try:
                response = ai_provider.post_prompt(
                    test_prompt, system_message="You are a biblical scholar and literary analyst."
                )
...
def demonstrate_integration_with_prophecy():
    ...
    try:
        ...
        print("-" * 50)
        print(
            populated_template[:400] + "..."
            if len(populated_template) > 400
            else populated_template
        )
        print("-" * 50)
