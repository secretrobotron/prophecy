...
class Bible:
    ...
    def _load_book(self, book_title: str) -> Dict:
        ...
        if book_title not in self.index:
            ...
            raise ValueError(f"Book '{book_title}' not found in index. Available books: {available_books}")
    ...
    def _parse_verse_range(self, verse_range: str) -> tuple:
        ...
        if not match:
            raise ValueError(f"Invalid verse range format: '{verse_range}'. Expected format: 'chapter:verse-chapter:verse'")
    ...
    def _extract_text_from_range(self, book_data: Dict, start: tuple, end: tuple) -> str:
        ...
        if (start_chapter > end_chapter) or (start_chapter == end_chapter and start_verse > end_verse):
            raise ValueError(f"Invalid range: start ({start_chapter}:{start_verse}) must be <= end ({end_chapter}:{end_verse})")
        ...
        if not text_parts:
            raise ValueError(f"No verses found in range {start_chapter}:{start_verse}-{end_chapter}:{end_verse}")
    ...
    def get_text(self, book_title: str, *parts: Dict[str, Union[int, str]]) -> str:
        ...
        for part in parts:
            ...
            elif all(key in part for key in ['start_chapter', 'start_verse', 'end_chapter', 'end_verse']):
