...
class TestStoriesStructure:
    ...
    def test_stories_are_top_level_entries(self, stories_data):
        ...
        for story_title in stories_data.keys():
            ...
            assert len(story_title.strip()) > 0, (
                f"Story title '{story_title}' is empty or whitespace"
            )
    ...
    def test_each_story_has_required_fields(self, stories_data):
        ...
        for story_title, story_data in stories_data.items():
            ...
            assert isinstance(story_data["book"], str), (
                f"Story '{story_title}' book field is not a string"
            )
            assert len(story_data["book"].strip()) > 0, (
                f"Story '{story_title}' has empty book field"
            )

            # Check 'verses' field exists and is a list
            assert "verses" in story_data, f"Story '{story_title}' missing 'verses' field"
            assert isinstance(story_data["verses"], list), (
                f"Story '{story_title}' verses field is not a list"
            )
    ...
    def test_verse_ranges_exist_in_bible_books(self, stories_data, bible_books_cache):
        ...
        for story_title, story_data in stories_data.items():
            ...
            for verse_range in verses:
                ...
                self._validate_verse_reference(
                    story_title, book_name, book_data, end_chapter, end_verse, verse_range, "end"
                )

                # Validate that start <= end (logically)
                assert (start_chapter < end_chapter) or (
    ...
    def _validate_verse_reference(
        self, story_title, book_name, book_data, chapter_num, verse_num, verse_range, position
    ):
    ...
    def test_comprehensive_stories_coverage(self, stories_data, index):
        ...
        for book in referenced_books:
            assert book in index, f"Referenced book '{book}' not in Hebrew Bible index"
